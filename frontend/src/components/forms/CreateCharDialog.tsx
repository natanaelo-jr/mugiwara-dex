...
const CreateCharDialog: React.FC<Props> = ({ children }) => {
  ...
  const {
    ...
    handleSubmit: handleSubmitPirate,
    formState: { errors: errorsPirate },
  } = pirateForm;
  const {
    register: registerMarine,
    handleSubmit: handleSubmitMarine,
    formState: { errors: errorsMarine },
  } = marineForm;
  ...
  const onSubmit = async (data: pirateData | marineData) => {
    ...
    api
      ...
      .catch((error) => {
        console.error(error);
        if (typePirate) {
          processPirateErrors(error.response?.data);
        } else {
          processMarineErrors(error.response?.data);
        }
      });
  };
  useEffect(() => {
    if (typePirate) {
      processPirateErrors(errorsPirate);
    } else {
      processMarineErrors(errorsMarine);
    }
  }, [typePirate, errorsPirate, errorsMarine]);

  const renderHakiCheckbox = (
    field: any,
    label: string,
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="sm:max-w-[40vw] bg-zinc-300">
          <form
            onSubmit={
              typePirate
                ? handleSubmitPirate(onSubmit, onInvalid)
                : handleSubmitMarine(onSubmit, onInvalid)
            }
          >
            <DialogHeader>
            ...
            <div className="flex w-full items-end gap-2 flex-col">
                ...
                <div className="flex flex-col w-full justify-between">
                  <div className="flex items-center gap-2 pr-2">
                    <div className="w-20 flex-col">
                      <span className="text-xs w-full font-bold text-zinc-700 ml-3">
                        Idade
                      </span>
                      <Input
                        variant="insertform"
                        ...
                        min={0}
                        {...(typePirate
                          ? registerPirate("age", { valueAsNumber: true })
                          : registerMarine("age", { valueAsNumber: true }))}
                      />
                    </div>
                    <div className="w-full">
                      {typePirate ? (
                        <div>
                          <span className="text-xs font-bold text-zinc-700 ml-3 w-40">
                            Recompensa
                          </span>
                          <Input
                            variant="insertform"
                          ...
                          <Controller
                            ...
                            render={({ field }) => (
                              <Selector
                                ...
                                value={
                                  inversePositionsMap.get(field.value) ||
                                  "Marinheiro"
                                }
                                onSelect={(v) =>
                                  field.onChange(
                                    positionsMap.get(v) || "Marine",
                                  )
                                }
                              />
                            )}
                  ...
                  <div className="w-full h-full flex flex-col justify-center py-2">
                    <span className="text-xs font-semibold text-zinc-700 ml-3">
                      Haki
                    </span>
                        ...
                        <Controller
                          ...
                          render={({ field }) =>
                            renderHakiCheckbox(
                              field,
                              "Observação",
                              <Eye className="text-zinc-700 w-5 h-5" />,
                            )
                          }
                        />
                      ) : (
                        ...
                        <Controller
                          ...
                          render={({ field }) =>
                            renderHakiCheckbox(
                              field,
                              "Armamento",
                              <Sword className="text-zinc-700 w-5 h-5" />,
                            )
                          }
                        />
                      ) : (
                        ...
                        <Controller
                          ...
                          render={({ field }) =>
                            renderHakiCheckbox(
                              field,
                              "Conquistador",
                              <Crown className="text-zinc-700 w-5 h-5" />,
                            )
                          }
                        />
                      ) : (
              ...
              <div className="w-full flex gap-4 justify-between">
                <div className="flex w-full flex-col">
                  <span className="text-xs ml-3 text-zinc-700 font-semibold">
                    Fruta
                  </span>
                  {typePirate ? (
                    <Controller
